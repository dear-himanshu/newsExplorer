/* 
Clone this neetoCode and use the data given below to 
build the Real Time News Explorer App
*/
const data = [
    {
      category: "business",
...
data.sort((a, b) => dateTimeFunc(b.dateAndTime,1) - dateTimeFunc(a.dateAndTime,1));
...
let status_array = ["1","0","0","0","0"];

// console.log(status_array);

for(let i = 0 ; i < numberOfArticles ; i++){
    const news_article_box = document.createElement("div");

    newsContainer.appendChild(news_article_box);
    news_article_box.classList.add(`news${i}`);
    news_article_box.classList.add("btn_event");
    news_article_box.setAttribute("category",data[i]["category"]);
    // news_article_box.textContent = "Dsfsfsdfsd";
    // console.log(news_article_box.getAttribute("category"));
    ...
    heading_div.classList.add("title_of_the_news");

    // const date_contents = data[i]["dateAndTime"].split("/",",",":","");
    const date_contents = data[i]["dateAndTime"].split(/[\/,:]/);
    // console.log(date_contents);

    let [d,m,y] = dateTimeFunc(data[i]["dateAndTime"],0);    
    let suffix = getDaySuffix(d);
    let month_value = getmonth(m);

    
    p_heading.textContent = data[i]["title"];
    // p_date.textContent = data[i]["dateAndTime"];
    p_date.textContent = `${d}${suffix} ${month_value} ${y}`;
    p_content.textContent = data[i]["content"];


    heading_div.appendChild(p_heading);
    date_div.appendChild(p_date);
    content_div.appendChild(p_content);

    news_article_box.appendChild(heading_div);
    news_article_box.appendChild(date_div);
    news_article_box.appendChild(content_div);

    // Very crucial line , remember this 
    news_article_box.classList.add("show_class");
    

    if(i>6){
        news_article_box.classList.add("handle_click");
    }

    // btn.addEventListener("click", ()=>{

    //     news_article_box.classList.toggle("show_class");
    // })

}

// Checking filter button status and toggling use

const fiter_btn = document.querySelectorAll(".filter_menu_elements button")
const all_btn = document.querySelector(".all");
const b_btn = document.querySelector(".business");
const p_btn = document.querySelector(".politics");
const s_btn = document.querySelector(".sports");
const e_btn = document.querySelector(".entertainment");

let visible_news = Array.from(document.querySelectorAll(".btn_event"));
// console.log(visible_news);

const function_visible = () => {
  visible_news.forEach(news => {
    if(userName.value.trim() === ""){

    let var1 = news.getAttribute("category");
    
    if(var1 === "business" && status_array[1] === "1"){
      // news.style.display = "inline";
      news.classList.remove("hide_class");
      news.classList.add("show_class");
      
      // console.log(var1);
    }
    else if(var1 === "politics"  && status_array[2] === "1"){
      // news.style.display = "inline";
      news.classList.remove("hide_class");
      news.classList.add("show_class");
    ...
    else if(var1 === "sport"  && status_array[3] === "1"){
      // news.style.display = "inline";
      news.classList.remove("hide_class");
      news.classList.add("show_class");
    ...
    else if(var1 === "entertainment"  && status_array[4] === "1"){
      // news.style.display = "inline";
      news.classList.remove("hide_class");
      news.classList.add("show_class");
      ...
      news.classList.remove("show_class");
      news.classList.add("hide_class");
    }

    // to check whether filters are working fine or not 
    // console.log(visible_news.filter(item => item.style.display === "inline").length);
    
    // if(news.style.display !== "none" && v===0){
    //   news.style.gridColumn = "1/-1";
    //   v++;
    // }
    // else if(v!==0){
    //   news.style.gridColumn = "0";
    // }

  }
  else{
    debouncedGetData(userName.value.trim());
  }
  });
...
fiter_btn.forEach(filter => {
  filter.addEventListener("click", ()=>{
    let filter_tag = filter.getAttribute("status") ; 
    ...
    if(filter_tag==="0"){
      ...
      if(filter.className!=="all"){
        // console.log("hsdfsf");
        all_btn.setAttribute("status","0");
        all_btn.style.backgroundColor = "#E0E0E0";
        all_btn.style.color = "black";
      }
      else{
        p_btn.setAttribute("status","0");
        p_btn.style.backgroundColor = "#E0E0E0";
        p_btn.style.color = "black";

        b_btn.setAttribute("status","0");
        b_btn.style.backgroundColor = "#E0E0E0";
        b_btn.style.color = "black";

        s_btn.setAttribute("status","0");
        s_btn.style.backgroundColor = "#E0E0E0";
        s_btn.style.color = "black";

        e_btn.setAttribute("status","0");
        e_btn.style.backgroundColor = "#E0E0E0";
        e_btn.style.color = "black";
      }
    }
    else{
      filter.setAttribute("status", "0") ;
      filter.style.backgroundColor = "#E0E0E0";
      filter.style.color = "black";

    }
    // console.log(filter.getAttribute("status"));

    status_array = [all_btn.getAttribute("status"),b_btn.getAttribute("status"),p_btn.getAttribute("status"),s_btn.getAttribute("status"),e_btn.getAttribute("status")]
    
    if(status_array[0]==="0" && status_array[1]==="0" && status_array[2]==="0" && status_array[3]==="0" && status_array[4]==="0"){
      all_btn.setAttribute("status","1");
    ...
    // (news["category"] === "business" && status_array[1]) || (news["category"] === "politics"  && status_array[2]) || (news["category"] === "sports"  && status_array[3]) ||(news["category"] === "entertainment"  && status_array[4]) || status_array[0] === true


    // useless operation no need to filter the data  

    let new_data = data.filter(news => (news["category"] === "business" && status_array[1] === "1") || (news["category"] === "politics"  && status_array[2] === "1") || (news["category"] === "sport"  && status_array[3] === "1") ||(news["category"] === "entertainment"  && status_array[4] === "1") || status_array[0] === "1") ;
...
const userName = document.querySelector(".input_area");
...
const getUser = (query) => {
  Array.from(newsContainer.children).forEach((curElem) => {

    let ce = curElem.querySelector("div p");
    // let news1 =  (news["category"] === "business" && status_array[1] === "1") || (news["category"] === "politics"  && status_array[2] === "1") || (news["category"] === "sport"  && status_array[3] === "1") ||(news["category"] === "entertainment"  && status_array[4] === "1") || status_array[0] === "1" ; 

    // ce.innerText.toLowerCase().includes(query.toLowerCase()) ?
    // curElem.classList.remove('hide_class') :
    // curElem.classList.add('hide_class') ;    //does not effect any thing incase the class does not exit
    let var2 = curElem.getAttribute("category");
    let pole = 0 ;
    if(var2 === "business" && status_array[1] === "1"){
      pole =1 ;
    }
    else if(var2 === "politics"  && status_array[2] === "1"){
      pole =1 ;

    }
    else if(var2 === "sport"  && status_array[3] === "1"){
      pole =1 ;

    }
    else if(var2 === "entertainment"  && status_array[4] === "1"){
      pole =1 ;
    ...
    if (ce.innerText.toLowerCase().includes(query.toLowerCase()) && pole ) {

      curElem.classList.remove('hide_class');
      curElem.classList.add('show_class');
      let highlightedText = ce.innerText.replace(new RegExp(query, 'gi'), `<span style="color: #1A5319;">$&</span>`);
      ce.innerHTML = highlightedText;

    } else {
      curElem.classList.remove('show_class');
      curElem.classList.add('hide_class');
      ce.innerText.color = "black";
    }
...
userName.addEventListener('keyup', (event)=>{
  // if(event.target.value.trim()!==""){
    debouncedGetData(event.target.value.trim());
  // }
})


// fiter_btn.forEach(btn => {
//   btn.addEventListener("click", ()=>{
//     debouncedGetData(userName.value);
//   })
// })
...
document.querySelector(".btn_show").addEventListener("click", ()=>{
  // console.log("2");
  all_element = document.querySelectorAll(".news_section div");
  // console.log(all_element.length);
  
  
  all_element.forEach(ele =>{
    // if(all_element.length>7 ){
    //   ele.style.color = "red";
    // }
    ele.classList.remove("handle_click");
